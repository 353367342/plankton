...
features:add(nn.SpatialConvolutionMM(fSize[1],fSize[2],10,10,2,2)) -- (120 - 10 + 2)/2 = 66
features:add(nn.Threshold(0,1e-6))
features:add(nn.SpatialMaxPooling(2,2)) -- 33
features:add(nn.SpatialConvolutionMM(fSize[2],fSize[3],7,7)) -- 27
features:add(nn.Threshold(0,1e-6))
features:add(nn.SpatialMaxPooling(2,2)) -- 13.5
features:add(nn.SpatialConvolutionMM(fSize[3],fSize[4],5,5)) -- 6.5
features:add(nn.Threshold(0,1e-6))
...
mdl:add(nn.ReLU())
mdl:add(nn.Linear(featuresOut,hiddenNodes[1]))
mdl:add(nn.ReLU())
mdl:add(nn.Dropout(0.5))
mdl:add(nn.Linear(hiddenNodes[1],hiddenNodes[1]))
mdl:add(nn.ReLU())
mdl:add(nn.Dropout(0.5))
mdl:add(nn.Linear(hiddenNodes[1],hiddenNodes[2]))
